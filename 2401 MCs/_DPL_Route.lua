...
do
    ...
    function onLBSimulatorTick(simulator, ticks)
        ...
        simulator:setInputBool(3, simulator:getIsClicked(2))
        simulator:setInputBool(4, simulator:getIsClicked(3))
        simulator:setInputBool(5, simulator:getIsClicked(4))
        simulator:setInputBool(6, simulator:getIsClicked(5))
        simulator:setInputBool(7, simulator:getIsClicked(6))
        simulator:setInputBool(10, simulator:getIsClicked(7))
        simulator:setInputBool(11, simulator:getIsClicked(8))
...
function RouteConfig()
    return
    {
        ...
        BTN_NEXT = button(5, 50, 23, 8, BTN_OK_COLOR, "NEXT"),
        BTN_PREV = button(35, 50, 23, 8, BTN_CLOSE_COLOR, "PREV"),

        BTN_ADD = button(43, 20, 18, 8, BTN_OK_COLOR, "ADD"),
        BTN_RMV = button(43, 30, 18, 8, BTN_OK_COLOR, "RMV"),
        BTN_RST = button(43, 40, 18, 8, BTN_OK_COLOR, "RST"),


        IS_PRESSED = false,
        IS_PRESSED_CONF = false,
        IS_PRESSED_ROUTE = false,

        LABEL_POS_FIRST = {3, 7},

        ROUTE_INDEX = 1, -- next stop

        init = function(this)
            this.STATIONS["arctic"] = STATIONS_ARCTIC
            this.STATIONS["sawyer"] = STATIONS_SAWYER
            this.STATIONS["meier"] = STATIONS_MEIER
            this.STATIONS["nso"] = STATIONS_NSO
        ...
        onTickUpdate = function(this, isTouched, inputX, inputY)
            ...
            if(not (prevPage or nextPage) and this.IS_PRESSED)then
                this.IS_PRESSED = false
            end

            if(this.PAGE == 0)then -- route page
                if ((isTouched and this.BTN_NEXT:isClicked(inputX, inputY)) and this.ROUTE_INDEX < #this.ROUTE and not this.IS_PRESSED_ROUTE) then
                    this.ROUTE_INDEX = this.ROUTE_INDEX + 1
                    this.IS_PRESSED_ROUTE = true

                elseif ((isTouched and this.BTN_PREV:isClicked(inputX, inputY)) and this.ROUTE_INDEX > 1 and not this.IS_PRESSED_ROUTE) then
                    this.ROUTE_INDEX = this.ROUTE_INDEX - 1
                    this.IS_PRESSED_ROUTE = true

               end
        
               if(not(isTouched and (this.BTN_NEXT:isClicked(inputX, inputY) or this.BTN_PREV:isClicked(inputX, inputY))) and this.IS_PRESSED_ROUTE)then
                    this.IS_PRESSED_ROUTE = false

               end

            elseif(this.PAGE == 1)then -- conf page
                ...
                if(not (prevList or nextList) and this.IS_PRESSED_CONF)then
                    this.IS_PRESSED_CONF = false
                end
            end

            return this.ROUTE_INDEX -- debug
        end,
        ...
        drawRoute = function(this)
            ...
            if(this.ROUTE == nil or #this.ROUTE == 0)then
                screen.drawRectF(this.LABEL_POS_FIRST[1], this.LABEL_POS_FIRST[2] + 6, 3, 3)
                screen.drawText(this.LABEL_POS_FIRST[1] + 6, this.LABEL_POS_FIRST[2] + 5, "N/")
                screen.drawText(this.LABEL_POS_FIRST[1] + 15, this.LABEL_POS_FIRST[2] + 5, "A")
                ...
                if(this.ROUTE_INDEX <= 3)then -- case 1 : first 3
                    while #displayList < 5 and localIndex <= #this.ROUTE do
                        displayList[localIndex] = this.ROUTE[localIndex]
                    ...
                    -- 2 next
                    offset = -2
                    while (#displayList < 3 + math.clamp(#this.ROUTE - this.ROUTE_INDEX, 0, 2)) and localIndex <= #this.ROUTE do
                        displayList[localIndex] = this.ROUTE[this.ROUTE_INDEX + offset]
                        localIndex = localIndex + 1
                        offset = offset + 1
                    end
                ...
                for index, value in ipairs(displayList) do
                    setColor(DARK_THEME[3])

                    if(value == this.ROUTE[this.ROUTE_INDEX])then
                        setColor(BTN_CLOSE_COLOR[1])

                    elseif indexOf(this.ROUTE, value) < this.ROUTE_INDEX then
                        setColor({40, 40, 40})
                    end

                    screen.drawRectF(this.LABEL_POS_FIRST[1], this.LABEL_POS_FIRST[2] + 1 + (index * 5 + index), 3, 3) -- dot
                    screen.drawText(this.LABEL_POS_FIRST[1] + 5, this.LABEL_POS_FIRST[2] + (index * 5 + index), value)
                end
...
function onTick()
    ...
    index = main:onTickUpdate(isTouched, inputX, inputY)
    output.setNumber(10,index)
end
