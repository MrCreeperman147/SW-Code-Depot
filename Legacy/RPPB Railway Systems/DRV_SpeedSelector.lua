...
increment = property.getNumber("Selector Increment")

selector = counterUpDown(0, maxSpeed, 0)
currentSelect = 0

function onTick()
    up = input.getBool(16)
    down = input.getBool(17)
    reset = false

    if(reset and not (up or down))then
        currentSelect = selector:reset()

    elseif(up and not (down or reset))then
        currentSelect = selector:update(up, increment)

    elseif(down and not (up or reset))then
        currentSelect = selector:update(not down, increment)

    end

    output.setNumber(1, currentSelect)
end
