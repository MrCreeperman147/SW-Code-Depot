...
autoBrakeDisplay = screen.segmentDisplay(DARK_THEME[3], 2, 4, 5, 57, -1, 1, true)
speedDisplay = screen.radialDisplay({DARK_THEME[3], DARK_THEME[1]}, 26, 32, 15, 4, 0, 350, 5*math.pi/6)
throttleDisplay = screen.segmentDisplay(DARK_THEME[3], 44, 4, 5, 57, -1, 1, true)

function onTick()

    -- throttle/brake section inputs
    autoBrake = math.abs(input.getNumber(3))
    authorizedThrottle = math.abs(input.getNumber(32))

    -- radial display section inputs
    currentSpeed = math.abs(input.getNumber(31))
    targetSpeed = input.getNumber(30)
    throttleMode = input.getNumber(29)

    -- applied throttle section inputs
    appliedEffort = math.abs(input.getNumber(1))
    dynamicBrake = math.abs(input.getNumber(2))

    -- alerter section inputs
    parkingBrake = input.getBool(2)
    emergencyBrake = input.getBool(3)
    deadman = input.getBool(32)
    wheelSlip = input.getBool(31)
...
function onDraw()
    ...
    -- throttle/brake section (5px)
    autoBrakeDisplay:draw(-autoBrake)
    autoBrakeDisplay:draw(authorizedThrottle)
    ...
    screen.drawTextBox(8, 7, 36, 5, math.floor(currentSpeed) .. "KM/H", 0)

    screen.setColor(41, 73, 255)
    screen.drawTextBox(8, 55, 36, 5, math.floor(targetSpeed) .. "KM/H", 0)
    mode = "CRUISE"
    ...
    -- applied throttle section (5px)

    throttleDisplay:draw(-dynamicBrake)
    throttleDisplay:draw(appliedEffort)
